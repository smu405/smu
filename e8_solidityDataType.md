...
```pragma```는 컴파일할 수 있는 버전을 지정하는데 사용할 수 있다. 

또는 버전의 범위를 지정해서 컴파일할 수 있도록 하려면, 예를 들어 다음처럼 지정한다.

```
pragma solidity ^0.8.1;
```

여기서 지정한 버전 번호인 ^0.8.1의 '^' 표시는 이 소소 코드를 Solidity 컴파일러 0.8.1 버전 포함해서 그 이상 0.8의 버전에서는 컴파일할 수 있다는 의미이다. 단 상위 버전인 0.9.x 버전부터는 컴파일하지 못한다.

Solidity 컴파일러는 버전이 바뀌면서 기능이 많이 바뀔 수 있고, 기존 버전의 언어와 문법이 달라질 수 있다. 두 번째 버전 번호는 버그 수정 버전을 나타내는 숫자로, 기존 언어와의 호환성을 유지한다. 

따라서 \"^0.8.1\"은 0.8.1 미만 버전과 0.9.0 버전 이상에서는 컴파일 할 수 없음을 나타낸다. 하지만 0.8.1 이상의 0.8.x버전에서는 컴파일 할 수 있다. 
...
* ```unsigned int```는 0이상의 정수만 사용할 수 있다.
...
address는 객체로서 잔고를 읽거나 및 입출금이 가능하다. 입출금 계정은 ```address payable```로 선언해야 한다.
...
* tx는 컨트랙을 호출하는 트랜잭션 관련 정보들을 가지고 있다. 
    * ```tx.origin``` 트랜잭션에 사인한 계정
    * ```tx.gasprice``` 트랜잭션 호출자가 명시한 gas price

* msg는 컨트랙의 함수를 호출한 전송 관련 정보를 포함한다.
