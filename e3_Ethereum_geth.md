...
버전에 따라 차이가 있는데, 최신 버전에서는 ```--testnet``` 대신에 그 명칭을 사용해야 할 수도 있다. ```geth --ropsten```이라고 해야 한다. ```geth -h```에서 출력되는 도움말에 해당 스위치가 존재하는지 확인하면 된다.
...
<삭제> ![alt text](figures/1_gethClientRun.png "geth client run")
...
버전 1.10 이상을 사용한다면, rpc 대신 http로 교체되었으니, 아래와 같이 변경된대로 rpc 대신 http를 적어준다.
```
--http --http.addr "117.16.44.45" --http.port "8445" --http.corsdomain "*" \
...
유닉스 계열 운영체제에서는 로그를 조용하게 파일로 보내려고 하는 경우, 재지정 명령어를 추가할 수 있다. 아래의 맨 뒤 ```명령어 2>> 파일명 ```는 명령어에서 발생하는 오류 (0번은 STDIN, 1번은 STDOUT, 2번은 오류)를 파일로 **재지정 Redirection**하는 명령어이다. 부등호 ```>``` 는 1회, ```>>```는 추가하는 재지정이다.
...
--ws --ws.addr="117.16.44.45" --ws.port "8446" --ws.origins="" \
--datadir $_dir \
--port "38445" \
--networkid 33
...
앞에서 사설망을 구성하였다. 여기에 고정 IP 주소를 적도록 한다.
...
12. 공중망이 있는데 테스트망은 왜 필요한가? 어떻게 개설하나?
