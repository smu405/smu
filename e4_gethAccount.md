...
Geth는 블록체인에 대해 명령어를 입력하고 결과를 받아볼 수 있는 클라이언트이다. 이 콘솔창에서 eth, admin, miner, personal 모듈의 명령어를 배우게 된다. 블록체인에서 사용하게 되는 계정과 주소를 생성할 수 있게 되고, 디지털 서명, 해싱을 이해하게 된다.

# 1. Geth 모듈 명령어들

Geth는 **G**o 프로그래밍 언어로 구현된 **eth**ereum이라는 앞 글자를 따와서 이름이 붙여졌다. 앞서 배웠듯이, Geth를 실행하면 네트워크에 접속이 되고 현재 사용하는 컴퓨터를 이더리움 노드로 만든다.

그렇다고 Geth가 접속 도구로만 사용되는 것은 아니다. Geth는 연결한 블록체인에 대한 클라이언트 역할도 한다.

블록체인은 분산된 네트워크로 구성되어 있으며, 중앙 집중식 서버가 없다. 대신, 블록체인 자체를 서버로 간주하고 로컬 컴퓨터를 클라이언트로 생각하면 이해가 쉬울 수 있다. 단 서버는 서비스를 제공하고, 클라이언트는 이를 요청하는 그런 주종 관계가 아니라는 것은 분명히 이해하자. 분산시스템에서는 참여하는 모든 노드가 서버가 될 수 있기 때문이다.
...
명령창A에서  \"eth.\"를 입력한 후 점연산자 (```.```) 다음에 <TAB>키를 눌러보면 (```> eth.<TAB>```) (<TAB>키는 자동완성기능이다), eth 모듈에서 제공하는 API를 출력한다. 계정, 블록, gas, 트랜잭션, 디지털 사인 관련 명령을 제공한다.

```
명령창A의 pjt_dir> geth attach http://localhost:8445
geth> eth.<TAB>
...
> 더 알아보기: 코드 자동완성 <TAB>
> 자동 완성은 코드 입력 중에 사용자가 입력하는 내용을 기반으로 가능한 완성된 코드를 제안하는 기능이다. 명령어, 변수명, 함수명의 일부를 입력하고 탭키를 누르면 가능한 명령어나 옵션을 자동으로 완성해준다. 오타를 줄이고 빠르게 원하는 명령어를 입력할 수 있도록 도와준다.

### 계정 정보

3장에서 생성한 계정을 명령창A에서 출력해보자.  물론 계정은 여러 개 생성할 수 있고, 생성된 순서로 인덱스가 주어지고, 그 인덱스를 지정하면 특정 account를 조회할 수 있다.
...
계속해서 명령창A에서 계정의 잔고를 알아보자. 계좌를 새로 개설했으므로 잔고는 없다고 출력한다. 
```python
...
계좌에 잔고가 없으면 거래를 하거나, 프로그램을 블록체인에 올리거나 어떤 일도 할 수 없다.
...
main, testnet에서는 마이닝이 언제나 실행되고 있으므로 별도의 마이닝이 필요없겠지만, 사설망에서는 컴퓨터가 마이닝 작업으로 인해 느려지는 것을 막기 위해서 필요한 경우에만 마이닝을 켜고 끌 수가 있다.
...
### 블록 갯수
블록이 몇 개나 있는지 알아보자. 독자가 사용하는 컴퓨터에서는 다른 값이 나올 수 있겠지만, 여기서는 57개가 생성되었다고 출력되었다. 블록이 생성이 되었다는 것은 마이닝이 그만큼 완성되었다는 의미이고, 그 보상이 계좌에 입금된 것이다. 

coinbase 또는 etherbase는 입출금이 일어나는 자신의 주 계정을 말한다.
...
지금까지는 geth 콘솔 프롬프트 ```> ```에서 작업하는 것을 보았다.


geth 콘솔에서 한 줄씩 입력했던 내용은 사실 자바스크립트 코드였다. 이 명령문들을 묶어 저장해서 일괄 실행할 수 있다.
...
> 또한 윈도우 운영체제에서 실행한다고 하더라도, 유닉스의 디렉토리 구분자 (```/```)를 사용한다.

```python
pjt_dir> geth --exec "loadScript('src/ethCommands.js')" attach http://localhost:8445
```
...
```
pjt_dir> geth --jspath "src" --exec "loadScript('ethCommands.js')" attach http://localhost:8445
...
admin모듈은 Peer, RPC 등 노드 관리에 필요한 명령어를 제공한다. 마찬가지로 geth 프롬프트 ``` > ```에서 admin.하고 <TAB>을 누르면 제공되는 admin의 API를 볼 수 있다.
...
> admin.nodeInfo
{
  enode: "enode://94b49...생략...76963@xxx.xxx.xxx.xxx:38445",
...
블록체인에 연결되어 있는 다른 컴퓨터(peer)가 있다면 출력할 수 있다. 프롬프트에서 \"admin.peers\" 라고 실행시킬 수도 있지만, 명령행(command line)에서 geth로 다음과 같이 실행하고 출력할 수도 있다.

```python
pjt_dir> geth --exec admin.peers attach http://localhost:8445
[{
    caps: ["eth/62", "eth/63"],
    ...
    network: {
      inbound: false,
      localAddress: "117.xxx.xxx.xxx:35582",
...
**enode**는 이더리움 노드의 줄임말로, 이더리움 노드 간의 통신을 위한 고유의 식별자이다. 따라서 각 노드의 enode는 모두 서로 다르다.

인터넷에서 통신을 위해 사용하는 URL과 유사하게 고유의 ID로서, ```enode://<노드 ID>>@<IP 주소>:<포트>?discport=<검색 포트>```으로 구성된다.

-   `<노드 ID>`: 노드의 고유한 식별자, 이더리움 노드의 공개 키를 해시하여 생성된다 (64바이트).
-   `<ip_address>`: 노드의 IP 주소, 도메인명은 허용되지 않는다.
-   `<port>`: 노드가 P2P 통신을 위해 사용하는 포트 번호
-   `<discovery_port>`: 노드 검색(Discv5)을 위해 사용하는 포트 번호
...
이더리움에서는 peer를 발견하여 네트워크를 구성하는 방법은 다음과 같다.

- **Bootnode**에 연결하여 네트워크의 다른 노드들을 찾고 피어를 발견하게 된다.
- **Discovery 프로토콜:** 이더리움 노드는 Discovery 프로토콜을 사용하여 네트워크에서 다른 노드를 검색한다. 이 프로토콜은 UDP를 통해 작동하며, 노드는 랜덤한 노드들에게 쿼리를 보내어 네트워크를 탐색하고 새로운 피어를 발견한다.
- **Kademlia 프로토콜:** 이더리움에서는 Kademlia 프로토콜을 사용하여 노드들이 서로를 발견하고 통신한다. 이 프로토콜은 DHT(Distributed Hash Table)를 사용하여 노드들을 저장하고 검색한다. 새로운 노드는 DHT를 쿼리하여 가장 가까운 노드를 찾고, 해당 노드와 통신하여 블록체인 데이터를 교환한다.
- **Peer-to-Peer 연결:** 노드들은 발견된 피어들과 직접적으로 P2P 연결을 수립하고 데이터를 교환한다.

이더리움에서는 이러한 알고리듬에 따라 자동으로 발견하지만, 사설망에서 잘 되지 않으면 다음 방법으로 enode를 지정해서 peer를 인식할 수 있다.

* (1) ```geth --bootnodes``` 명령어로 시작할 때부터 Bootnode에 peer의 enode를 적어두거나,
...
**마이닝은 Target Hash를 맞출 때까지 계속 해시(Hash)를 생성하여 시도하는 과정**이다.
...
현재 coinbase를 ```accounts[1]```로 변경해보자. 이번에는 파일에 명령어를 입력하고 실행해 보자.
...
```python
pjt_dir> geth --exec "loadScript('src/ethMinerEtherbase.js')" attach http://localhost:8445
...
계정은 은행의 계좌라고 생각하면 쉽게 이해된다. 이더리움의 계정은 은행 계좌처럼 잔고를 가지고 있어 수금, 송금이 가능하다.

계정은 쉽게 만들 수 있다. 그러나 우리가 사용하는 은행 계좌와 달리 암호화되어 있기 때문에 단순하지 않고, 숫자와 문자가 섞인 난해하게 생긴 문자열로 이루어진다.
...
출금은 당연히 **디지털 서명**이 되어야 한다. 은행에서 입출금 거래를 하면서 서명 또는 날인을 하듯이 블록체인 거래도 그러하다. **디지털 서명**이 되어 있어야 거래로 인정된다.

거래가 위변조되면 디지털 서명이 맞지 않게 된다. 이 경우 새로 서명을 해야 한다. 누군가 악의적으로 원래의 디지털 서명을 사용하지 않으면, 대칭키인 공개키(public key)로 풀 수 없게 된다.

## 2.1 계정의 종류

이더리움에서 계정은 블록체인 안쪽 또는 바깥쪽에 있는지를 기준으로 **외부 계정** 또는 **내부 계정**으로 구분할 수 있다.

구분 | 설명
-----|-----
외부 계정 | 블록체인 외부의 계정, 사용자가 생성하고 소유하는 일반적인 계정을 말한다.
내부 계정 | 스마트 계약 내부의 계정, 스마트 계약이나 내부 계정이 생성한다. 

### 외부 계정

블록체인 **외부 액터가 가지는 계정(Externally Owned Account)**을 말한다. 외부 액터는 보통 개인이지만 법인도 물론 될 수 있다. 그 계정은 컴퓨터, 인터넷 또는 하드웨어에 존재할 수도 있다.

외부 계정은 고유의 **주소키**를 가지고 있어 트랜잭션을 **사인(sign)**할 수 있다. 어떤 물품을 구매한다면 계정에 있는 잔고에서 디지털사인을 하고 판매자에게 송금을 하게 된다.
...
내부계정은 외부에서 직접적으로 접근할 수 없으며, 외부계정과 달리 고유한 주소 키를 가지지 않는다.

대신, 내부 계정은 스마트 계약의 **프로그램 코드가 배포되고 계정이  생성**되고 스마트 계약 코드에 의해 조작된다. 외부에서 직접적으로 접근할 수 없고 스마트 계약이나 다른 내부계정 내에서만 사용된다.

내부계정은 외부에서 직접적으로 이더를 소유할 수 없으며, 스마트 계약의 상태 변경을 통해 자산을 가질 수 있기 때문에 코인을 발행할 수도 있다.

그러나 **외부 계정과 같이 고유의 주소키를 가지지 않아서** 트랜잭션에 사인할 수는 없다.

## 2.2 주소 키

이더리움은 한 계정에 하나의 주소 키를 가진다.

비유하면, 계정은 통장으로, 주소는 통장의 계좌번호와 비슷하다고 볼 수 있다. 그러나 주소 키는 블록체인의 분산화된 환경에서 사용자가 직접 소유하고 제어하는 반면, 은행의 계좌번호는 중앙화된 금융 시스템에서 은행이 관리하고 제어한다.

다음에서 보듯이 ```geth account new``` 명령어로 계정을 생성하면 주소 역시 생성된다.

> 주소와 계정

> **주소 address**는 **20 바이트 코드**로 계정 account의 고유 번호이다. 다른 말로 계정은 하나의 주소 식별자를 가진다. 자산을 전송하거나 수신할 때 사용되는 식별자이다.
> 계정은 잔고 (이더 또는 다른 암호화폐), 거래 건수를 가진다. 앞서 설명한 내부/외부 계정을 말한다.
...
cryptos나 bitcoin 라이브러리는 업그레이드가 몇 년이나 되지 않아서, **최신 버전의 파이썬과는 작동이 잘 되지 않는다**는 점을 주의한다. **현재 Python 3.7 버전**에서 하고 있다.
...
공개 키는 개인 키에서 생성한다. 암호학적으로 안전한 공개 키 알고리즘인 타원 곡선 암호**ECC(Elliptic Curve Crypotography)**를 사용하여 개인 키로부터 공개 키가 만들어진다.
...
>>> print(pubKey)
04f6cc26cec156c180f8...생략...4e87cf98001d428f564f 비트코인 공개키는 65바이트
```
...
개인 키는 일반적으로 16진수로 표현되지만, 이를 WIF(Wallet Import Format) 형식으로 변환하여 저장하고 관리할 수 있다.

WIF는 그냥 복잡해 보이는 개인 키를 **복사하거나 다루기 편리하게 다른 형식으로 표기하는 것**일 뿐이다. 원하면, **생성했던 절차를 반대로 수행해서 원래의 개인키로 변환, 회복**될 수 있다.
...
주소 파일은 이더리움 노드 data directory아래에 있다. 기억하겠지만 geth를 띄울 때 ```datadir``` 옵션에 해당하는 디렉토리를 말한다. 그 아래를 보면 키는 ```keystore``` 디렉토리 안에 `UTC--<<년>>-<<월>>--주소`와 같은 JSON 형식으로 저장되어 있다. 이 파일 안에는 private key, 비밀번호, 주소가 암호화되어 저장되어 있다.

아래는 리눅스에서 해당 파일의 내용을 출력하고 있다. 윈도우에서도 해당 디렉토리로 가서, 내용을 출력할 수 있다.

```python
pjt_dir> more eth\keystore\UTC--2019-01-03T21-57-03.966937024Z--21c704354d07f804bab01894e8b4eb4e0eba7451
...
블록체인에는 많은 계정이 존재하고 있다. 은행 계정과 같이 잔고를 얼마나 가지고 있는지 어떻게 추적할까?

State trie를 통해 모든 내부, 외부 계정의 현재 상태를 읽어올 수 있다.

계정을 키로 사용하고 (그래서 키는 32바이트), 이러한 키별로 4개의 속성 (1) nonce, (2) balance, (3) storageRoot, (4) codeHash를 가지고 있다.

![alt text](figures/4_stateTrie_2.png "stateTrie and storage Trie")

### (1) 계정 잔고

계정의 잔고는 어떻게 아는가?

이더리움에서 잔고(balance)는 Wei 단위로 저장된다. 앞서 설명한 ```eth.getBalance(주소키)```를 기억하자. 통장의 비밀번호와 같은 역할을 하는 키를 사용해 소유를 증명하고, 잔고를 읽을 수 있고 입출금을 할 수 있다. 

반면에 **비트코인**에서는 **거래의 기록에 그 잔고가 저장**된다. 거래를 하면 입금-출금의 잔액이 남게 된다. 그 잔액은 거래송신자의 주소로 돌려주게 된다. 이를 **계정의 미사용 잔액의 합계** UTXO(unspent transaction output)이라고 한다. 거래기록이 발생하면서 잔고기록은 여러 기록에 분산되게 된다. 입금이 발생하면 그 잔고가 합쳐져서 기록되는 것이 아니라, 건별 기록이 남는다. 따라서 잔고는 집계되어 있는 것이 아니라, 여러 기록에 남겨진 **미사용잔액 UTXO**을 여기 저기 찾아서 합산해야 한다. 비트코인은 통장이 있고, 그 계정의 거래만 그 통장에 기록되는 방식이 아니다.
...
### (3) storageRoot
StorageRoot는 계정이 저장된 해시 값이다. Storage trie는 컨트랙의 상태가 저장된다. 모든 계정은 각각 자신의 Storage trie를 가지고 있다. 그 Storage trie 루트의 256 비트 해시 값을 storageRoot라고 한다.

### (4) codeHash
스마트 계약 소스코드의 해시값이다.  codeHash는 외부계정에게는 이 값이 없고, 스마트 계약에게만 해당된다.

이 해시값은 해당 스마트 계약의 코드를 식별하는 역할을 한다. 스마트 계약의 소유자나 사용자가 해당 계약의 코드를 검증하거나 식별하는 용도로 사용된다. ```eth.getCode()``` API를 통해 스마트 계약의 `codeHash`를 조회할 수 있습니다.
...
입력 데이터의 **길이가 달라져도** 결과는 **일정한 크기**, 예를 들면 256비트의 문자열로 생성된다.

이 경우 $\frac{1}{2^{256}}$의 가능성이 거의 없는 맞출 확률이 존재한다.

해시의 특징은:

1. 해시는 **원본을 되살릴 수 없다**. 해시는 원본에서 알고리즘에 따라 원본의 길이와 무관하게 고정 길이로 생성해낸 값이라 원본을 되살리기 극히 어렵다. (역상 저항성, preimage resistance). 즉  메시지 m에서 h를 구했으면 h = hash(m), h를 안다고 m을 알아내지 못해야 한다.
2. 그러나 해시는 **입력 데이터가 동일하면** 즉 수정되지 않는 한 항상 **같은 고유 값**을 생성해 낸다 (deterministic)
3. 원본이  **아주 조금 변경되어도 해시 값은 완전히 다른 고유 값**이 생성된다.
4. 입력 데이터가 다르면 해시 값이 다르다. 다른 운영체제, 다른 하드웨어에서 해싱한다해도 우연히라도 같은 경우가 있을 수 없다 (충돌 저항성 collision resistance).서로 다른 메시지 m1, m2의 해시가 ```hash(m1) = hash(m2)``` 서로 같지 않도록 한다
...
2. **무결성 검증**에 사용한다. **거래가 원본**임을 확인할 때 디지털 서명된 해시값과 비교하여 변조를 확인한다.
...
```python
pjt_dir> geth --exec "web3.sha3(web3.toHex('Hello Hash0'))" attach http://localhost:8445
```

## 2.5 디지털 서명

디지털 서명은 메시지의 전송자를 인증하기 위해 사용된다. 디지털 서명은 원본을 변경하는 것은 아니라, 원본을 기반으로 생성을 해서 서명이 만들어진다.

송신측에서는:

- 메시지로부터 해시를 생성한다.
- 그 해시에 개인키를 적용해서 디지털 서명을 생성한다.

수신측에서는:

- 송신자가 해 놓은 디지털 서명을 공개키로 푼다. 당연히 풀리면 송신자가 맞다고 인증하게 된다.
- 메시지의 해시가 서로 일치하면 원본과 일치한다고 인증하게 된다.

예를 들어 갑이 을에게 메시지를 전송한다고 하자. 갑은 자신의 키로 메시지에 디지털 서명을 하고 전송한다. 물론 메시지는 서명하기 전에 해싱해 놓아야 한다. 을은 갑의 공개키를 이용하여 메시지를 열람한다.

이 과정을 통해:

- 원본은 해싱하고 디지털 서명을 해 놓았기 때문에 원본이 **위변조 되지 않았음을 증명**할 수 있다. **어떤 악의적인 사용자가 원본을 수정하려고 한다고해도, 해시 값이 달라지기 때문에 그런 시도는 불가능**하다.
...
개인키(private key)는 디지털 서명에 사용한다. **개인키로 서명**하고, 받는 측에서 공개키(public key)로 검증한다.
...
이더리움의 키는 ECDSA 알고리즘을 적용해서 만든다. 이 알고리즘은 해킹할 수 없을 정도로 견고하고, 그래서 블록체인은 매우 튼튼하고 해킹의 위험이 없다고 봐도 좋다.
...
함수의 그래프를 코드를 작성해서 그려볼 수 있다. 그러기 위해서 다음을 실행하려면 numpy, matplotlib을 설치해야 한다. 하나씩 설치하지 않고 한꺼번에 ```pip install numpy matplotlib``` 하면 된다.
...
이 프로그램을 실행하면, 아래 호리병을 뉘운 모양의 ECDSA 곡선이 그려진다.

### Elliptic curve 연산

공개키는 타원곡선에 속하는 점의 곱셈 연산으로 구하게 된다.
...
ECDSA에서 공개키를 구하는 계산 과정은 다음과 같다.

(1)   타원 곡선 위의 한 점인 G(Generator Point)를 선택한다. 이는 타원 곡선의 생성자로 미리 정의된 점이다.

(2)  비밀키를 사용하여 공개키를 계산한다. 이때 공개키는 G를 비밀키의 값만큼 반복적으로 더한 결과이다. 수학적으로는 다음과 같이 표현된다. 

```python
P=k*G
```

이 때 **k가 개인키**, **P가 공개키**가 된다. 시작점 'Generator' G에 정수k를 계속 더하면 또는 2를 곱해 나가면, 그 결과 값인 P는 커브에 존재하는 점으로 계산된다.
...
secp256k1은 암호화폐에서 사용하는 사용하는 특정한 타원곡선을 말한다. 이 타원 곡선은 비트코인, 이더리움 등 여러 암호화폐에서 디지털 서명 및 공개키 암호화를 위해 널리 적용된다.
...
### 파이썬으로 유한체 Elliptic curve 이해하기

파이썬으로 Elliptic Curve를 생성해보자.

그러기 위해서는 라이브러리 Pycoin을 설치해야 한다. 동일한 명칭으로 ```cdecker/pycoin```와 ```richardkiss/pycoin``` 2 개가 있으니, 주의해서 후자를 설치한다. **매뉴얼에 3.6, 3.7을 지원**한다고 했지만 최신 버전은 잘 설치가 되지 않는다. python2로 해보거나, python3에서는 pycoin 버전을 낮추어서 0.77로 해보자.

```python
pip install pycoin==0.77    # 파이썬 3를 사용하면 pycoin의 특정 버전 사용. 윈도우에서는 pip대신 pip3 사용
pip install python2-secrets # python2에서 사용하는 경우
```
...
"vrs"는 디지털 서명에서 사용되는 요소이다.

ECDSA에서 디지털 서명은 (r, s) 또는 (v,r,s) 형태로 표현한다:

-   v는 서명의 일부로서 서명의 유효성을 나타내는 값으로, 주로 27 또는 28이고,
-   r은 서명의 일부로서 X 좌표를 나타내며,
-   s는 서명의 일부로서 서명의 숫자값을 나타낸다.

v, r, s는 서명할 때 넣어주고, 이 값을 이용하여 공개키를 알아낼 수 있다. 곧 5장에서 배우게 될 거래를 구성하는 필드이다.

v, r, s를 출력해 볼 수 있는 라이브러리로 eth_keys가 있는데, 설치하려면 ```pip install eth_keys```하면 된다.
...
메시지를 서명하고 나서 그 결과를 출력하면, 65 바이트이다.
...
그리고 디지털 서명으로부터 v, r, s 값을 구할 수 있다.
```python
>>> print("v: ", signature.v) # v는 1바이트로 0,1,2의 값을 가질 수 있다 (또는 27,28).
v:  0
>>> print("r: ", signature.r) # 디지털 서명 64바이트에서 출력할 수 있다.
r:  92658050108416009874678855014183140825758716366670817325881885506266554472830
>>> print("s: ", signature.s) # 디지털 서명 64바이트에서 출력할 수 있다.
...
```python
pjt_dir> geth --datadir "./eth" --mine --minerthreads=4 --port 30445
```
...
Wallet은 우리가 일상생활에서 사용하는 현금, 신용카드 등을 보관하는 지갑과 비슷한데, 암호화폐를 보관하고 관리하기 위한 소프트웨어나 하드웨어 장치를 의미한다.

- 소프트웨어 지갑은 컴퓨터나 모바일 기기에 설치되며, 예시로는 MetaMask, MyEtherWallet, iOS나 Android에서 사용되는 Trust Wallet, Mist (2019년 3월 개발 중단), MyCrypto (https://mycrypto.com), 가장 널리 알려진 암호화폐거래소인 Coinbase의 지갑, 한국 카카오에서 제공하는 클레이튼 블록체인의 카이카스 지갑등이 있다.

- 하드웨어 지갑은 말 그대로 USB 스틱이나 하드웨어 장치에 개인 키를 저장하여 오프라인 환경에서 자산을 보호할 수 있다. Ledger Nano S, Trezor 등이 대표적인 하드웨어 지갑이다.

우리가 사용하는 Wallet은 **계정, 개인키**를 가지고 있는 파일 또는 데이터베이스로 되어 있다. 그러나 단순히 계정만을 가지고 있지 않고 **블록체인과 인터페이스**하는 등 다음과 같은 기능을 제공한다:

1.  **키 관리:** 사용자의 개인 키와 공개 키를 생성하고 보관한다. 키를 매번 완전히 새롭게 생성하지 않고 **HD(Hierarchical Deterministic)** 방식으로 하나의 마스터 시드 Seed 키에서 다수의 주소를 생성할 수 있는데, 이를 계층적 결정 지갑(HD Wallet)이라고 한다.
    
2.  **자산 보관:** 비트코인, 이더리움, 리플 등 암호화폐 자산을 안전하게 보관한다.

3.  거래 생성 및 관리: 암호화폐 거래를 생성하고 서명하거나 거래의 내역을 추적하고 관리한다.
    
4.  **네트워크 연결:** 블록체인 네트워크에 연결할 수 있다.

geth도 지갑으로 이러한 네트워크에 연결하고, 계정을 만들고, 입출금, 전송을 할 수 있다. wallet은 네트워크에 저장되어 있지 않고, 사용자 컴퓨터 <DATADIR>/keystore에 있다. 아래에서 보듯이 형식은 UTC--{year}-{month}--{account} 출력된다.

```
pjt_dir> dir eth\keystore (pjt_dir\eth\keystore 폴더에 설치된 경우)
03/22/2020  05:42 PM  491 UTC--2020-03-22T08-42-34.444935200Z--c8ea4...64c7c0
03/22/2020  05:46 PM  491 UTC--2020-03-22T08-46-05.181606000Z--4fa2c...6f398
03/28/2020  10:44 AM  491 UTC--2020-03-28T01-44-04.678799900Z--ce5cd...166a6
```

또는 geth 단말에서 ```personal.listWallets``` 명령어로 지갑의 계정 내용과 현재 잠금 상태를 출력할 수 있다.

```python
geth> personal.listWallets
[{
    accounts: [{
...
## 연습문제
1. 자신의 컴퓨터에서 계정은 하나가 아니라 복수를 생성할 수 있다. 계정을 정렬해서 순서가 결정된다. OX로 답하시오.
2. 잔고확인 함수 eth.getBalance()는 어떤 인자를 넘겨주어야 하는지, 계정0의 잔고를 계산하는 명령문을 적으시오.
3. coinbase는 몇 번째 계정을 말하는지 답하시오.
4. coinbase를 계정1로 변경하는 명령문을 작성하시오.
...
11. 개인키는 몇 바이트인지 적으시오.
12. 공개키는 몇 바이트인지 적으시오.
13. 주소(address)는 몇 바이트인지 적으시오.
14. 공개키를 생성하는 알고리즘 명칭을 적으시오.
15. 우리가 보는 개인키는 이미 WIF형식으로 만들어져 있다. 그 개인키를 원시 그대로 볼 수 없다. OX로 답하시오.
16. nonce는 리플레이 공격을 방어하기 위해 사용한다. OX로 답하시오.
17. 이더리움에서 해싱하는 방식을 적으시오.
18. 해싱은 입력이 동일하면, 항상 동일한 값을 생성한다. OX로 답하시오.
19. 해싱할 때 nonce값을 더하면, 해싱 문자열은 더한 부분만 수정된다. OX로 답하시오.
20. 이더리움의 디지털 서명은 공적키로 서명하고 사적키는 필요하지 않다. OX로 답하시오.
21. 이더리움은 익명이라서 지갑에는 사적키는 없고 잔고만 보유하고 있다. OX로 답하시오.
22. 지갑은 컴퓨터에만 있을 수 있다. OX로 답하시오.
23. 주소를 생성할 때, 매번 seed를 가지고 생성할 수 있지만, 그렇지 않고 하나의 마스터 시드에서 생성할 수 없다. OX로 답하시오.
24. 이더리움에서는 비대칭키로 암호화를 한다. OX로 답하시오.
