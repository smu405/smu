...
블록체인의 거래(transaction)는 우리가 자주 사용하는 온라인 쇼핑의 거래와 비슷하다.

~~블록체인의 거래에 대해서 설명 추가가 필요합니다. 그래야 다음 줄에 있는 블록체인의 거래와 다를 바 없다는 말이 연결됩니다~~
...
온라인 쇼핑의 거래는 주문내역이 텍스트 형태로 인코딩되고, 블록체인에서는 이진수로 인코딩된 바이트코드로 전송된다. 인코딩하는 방식이 다를 뿐이다.~~URL인코딩된다는 뜻이 정확하게 모르겠습니다~~
...
이더리움에서의 거래는 블록체인에 기록이 되는지, 안되는지에 따라 구분한다.~~기록되지 않는 거래를 거래라고 할 수 있다는건가요? 기록되지 않는 것과 기록되는 것을 모두 거래라고 설명하는데, 다음 줄에서는 기록되지 않는 메시지 호출과 거래를 구별하고 있어서요(여기서의 거래는 기록되는 거래)~~
...
다음에 보인 것처럼 web3.sha3() 함수와 substring() 함수로 앞 4 바이트를 선택할 수 있다. 0x를 포함하므로 10개를 분리하면 된다.
